...
export default function Carrousel({ title, pictures }) {
  ...
  if (!title || !pictures || pictures.length === 0) {
    console.error('Carrousel component: title and pictures props are required.')
    return null
  }
  ...
  return (
    <section className={styles.carrousel} aria-label={title}>
      <img className={styles.carrousel_image} src={pictures[index]} alt={title} />
      {pictures.length > 1 && (
        <>
          <div className={styles.carrousel_vector_left} onClick={handleClickLeft}></div>
          <div className={styles.carrousel_vector_right} onClick={handleClickRight}></div>
